...
func locateLicense(inFolder folder: String) -> String? {
    ...
    var filteredPaths = subpaths.filter {
        $0.ends(with: "LICENSE") || $0.ends(with: "LICENSE.txt") || $0.ends(with: "LICENSE.md")
    }
...
libraries.forEach({ (library: String) in
    ...
    if library == "SimulatorStatusMagic" {
        return
    }
...
var plist: NSData = NSData()
do {
    plist = try PropertyListSerialization.data(fromPropertyList: licenses, format: .xml, options: 0) as NSData
} catch {
    print("Error getting plist data: \(error)")
    exit(1)
}
